...
def test_v9_drift_detector_names_missing_goal() -> None:
    ...
    report = analyze({Kind.PRD: PRD_3G, Kind.SLICES: SLICES_2_OF_3})
...
def test_full_coverage_is_clean() -> None:
    report = analyze({Kind.PRD: PRD_3G, Kind.SLICES: SLICES_3_OF_3})
...
def test_direct_covers_line_satisfies_coverage() -> None:
    """A slice using `**Covers:** Gn` directly counts as covering Gn."""
    ...
    report = analyze({Kind.PRD: PRD_3G, Kind.SLICES: slices})
    assert report.is_clean()
...
def test_drift_validation_covers_unknown_goal() -> None:
    ...
    report = analyze({Kind.PRD: prd, Kind.SLICES: SLICES_3_OF_3})
...
def test_idempotence_same_input_same_report() -> None:
    a = analyze({Kind.PRD: PRD_3G, Kind.SLICES: SLICES_2_OF_3})
    b = analyze({Kind.PRD: PRD_3G, Kind.SLICES: SLICES_2_OF_3})
